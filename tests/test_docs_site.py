...
2. every design note carries at least one `>>>` block, so a page cannot become
   ...
   nothing, stays green;
3. every user-guide page's `{code-cell}` blocks execute, in order, in one shared
   namespace. A page's outputs are generated by myst-nb rather than written
...
def test_guide_pages_exist() -> None:
    """The user-guide directory is not empty and is reachable from its index."""
    assert _GUIDE_PAGES, "docs/user-guide/ has no pages"
    index = (_DOCS / "user-guide" / "index.md").read_text()
...
def test_guide_page_executes(page: Path) -> None:
    ...
    A page's outputs are generated by myst-nb rather than written down, so
    ...
    assert cells, f"{page.name} carries no `{{code-cell}}`; user-guide pages are executable"
