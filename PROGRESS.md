...
| M14.1: ConfigSpace and SMAC3 bindings (the foreign-representation hand-off; static-count lifts, conditional activity, hard constraints as forbidden clauses with every miss reported) | 2026-08-17 | 3900 + 128 |
...
| Date | Fix | Against |
|---|---|---|
| 2026-08-18 | The Optuna and CMA-ES bindings answer for a subset's declared size too. Optuna withholds a flag the bound has settled, as it already withholds an inactive parameter; CMA-ES refuses the parameter by path, its flags being independent variables in a layout fixed before the first generation. `_placement` states the bound once, and states there that a binding either places it or refuses | M13.11 |
| 2026-08-17 | A subset's declared size survives being placed as one flag per item, the flags admitting every combination on their own. The ConfigSpace binding spells the bound out as the combinations it excludes, refusing by path past `MAX_SUBSET_CLAUSES`, and seeds the flags so its own clauses do not forbid the default configuration. The corpus fixtures carrying a bounded subset now run under the backend suite, which is what let this through | M14.1 |
| 2026-08-17 | A lift's determined count is a fact about the space, so a partial walk records it as it reaches it and an element condition reading a sibling of its own instance, a lifted choice's discriminator or a struct field's `when`, resolves against the instance leaves alone. A driver loop accumulating its own flat config writes no container key, that key being `flatten`'s bookkeeping, and without one every such payload read as inactive: `next_assignable` returned nothing while `param_activity` called the payload active, so the loop halted on a configuration `is_complete` refused, against the `driver_loop_coincidence` law. The Optuna binding returned that configuration for `memetic_pipeline`, the corpus fixture no backend test ran | M4, M6, M13.11 |
