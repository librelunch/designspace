...
something *you* or the solver author does — and there are exactly three shapes
it can take.
...
perturbation — mutate in u-space, decode through the chart:
...
applies to it unchanged — which is the whole point: a bridge does not introduce
a new negotiation vocabulary, it just moves where shape 1 gets applied.
...
2. a `Representation` whose target this solver can handle — geometry authored by
   the type author, with the loss declared rather than silent;
...
generation can still be rich to modeling — these do not trade off.

## Adapter conventions

Strategy-entangled operations — crossover schemes, mutation policies, trust
regions — are the only things genuinely forced into adapters. When you write
one:
...
- scope it per *(capability, type)* — not per *(solver, type)*, which multiplies
