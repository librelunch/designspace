...
| Method | Value | Notes |
|---|---|---|
| `.symbolic(signature, primitives, max_depth, validators=None, sampler=None)` | `{"ast", "source"}` | Structured expression trees. Variables auto-derived from `signature.args`. **Non-generative** unless `sampler=` opts in a generator (non-serializable; tree genomes are genotypes — solver territory otherwise). `validators` are callables over the AST. |
| `.code(signature, description="", constraints=None, examples=None, validators=None)` | `{"source": str}` | Freeform source. **Always non-generative** (no `sampler=` form). `validators` are callables over the source string. `description`/`constraints`/`examples` are declared, serialized, fingerprinted metadata for a consumer's own backend — core never interprets them. |

Core's job is narrower than it looks: *declare* the space, *validate* a submitted value's structure against that declaration, and carry it through every existing surface — never generate or evaluate one (tree/program generation is Out of Scope; see "Solver Integration").

**`.symbolic()`'s value.** `{"ast": <node>, "source": <str>}` — `"source"` is optional (a string when present, never cross-checked against `"ast"`: core owns no printer/parser, and rendering is not what makes a value valid). A node is one of:
...
A value validates when: every `"op"` name is one this param declared (a bare string in `primitives`, or a `Primitive`'s `name`); every `"var"` name is a key of `signature.args`; every `"const"` lies within the bounds of some declared `FloatLiteral` or `IntLiteral` (with none declared, `{"const": ...}` is never valid); and tree depth is `<= max_depth` (a leaf is depth 1). `validators` run after this structural check, over the AST; a validator returning `False` invalidates the value, and a raising validator is treated the same as a malformed value (never escapes a public call).

**Primitive vocabulary is open; arity binds only where declared.** A bare string in `primitives` names a token this param's trees may use — core assigns it no arity and no meaning. `ds.Primitive(name, arity, fn=None)` is the mechanism for pinning one: `arity` is an int (exact) or a `(lo, hi)` pair (`hi=None` unbounded), checked against a matching `{"op": name, ...}` node's `len(args)`; an int and its `(n, n)` spelling are fingerprint-equal (sugar-equivalence). A `Primitive` may shadow a built-in-sounding name — that is the supported way to pin one's arity. `fn`, if given, is never called by core (no evaluator ships; a consumer's own interpreter uses it) and rides the non-serializable set below.

**`.code()`'s value.** `{"source": <str>}`, `"source"` required. `description`/`constraints`/`examples` are plain declared metadata (LLM backends and other program-generation strategies are Out of Scope); `examples` entries must be JSON-serializable (row 23's rule).

**Generativity.** `.code()` is always non-generative. `.symbolic()` is non-generative unless `sampler=` is given — see "Sampling and Generativity" for the shared non-generative-materialization rule every such param (custom included) follows. `.freeze()` pins a program param via `require(p == value)` plus `default = value`, the same mechanism `.custom()` uses (see "Space — Structural Operations"); unlike `.custom()`, there is no shorthand exception — a program value is always a plain, comparable, serializable JSON dict, so `.slice()` supports it too (custom's `.slice()` rejection is specific to `.prop()`, which no program param has).
...
- **Container elements are bounded to one lift level.** A `struct`- or `choice`-typed element may sit under **at most one** `.repeat()`; scalar, subset, and permutation elements nest arbitrarily (`mask.repeat(8).repeat(8)` is legal). Two levels is a resolution error (row 34) — the boundary is the merged *shape*, not the syntax that reaches it, so both the chained spelling (`.space(...).repeat(3).repeat(2)`) and the **compositional** one (a struct/choice lift declared inside another lift's element `Space`, composing to `rows[].spans[].lo`) are rejected identically. This is a scope boundary, not a semantic one: the per-instance expansion that turns a `"[]"` template into `"[k]"` entries is single-level, and without the guard the shape resolves but never materializes its inner elements — a struct lift samples empty element dicts, and a lifted choice samples an empty payload that `validate()` then accepts.
...
1. **Leaves.** Any predicate or arithmetic term over an *inactive* param is Unknown. `is_active()` is the sole total predicate. Projection over an *inactive* lift is Unknown — distinct from an *active empty* list (below). A `ds.value(...)` node is Unknown iff **evaluating** one of its operands yields Unknown (joining provenance per rule 5 when more than one does) — not a literal scan of the referenced params' activity — so `.if_inactive()` and any other coercion inside an operand actually composes, exactly as *Expressions* describes; `fn` is not called in that case, so an opaque leaf obeys the same rule as a transparent one. An exception `fn` itself raises propagates uncaught — the calling convention (exactly the operand values, never the config) is the whole contract, and swallowing a misbehaving `fn` would undermine it (D-76).
2. **Propagation.**
...
.has_nongenerative_params -> bool            # replaces has_code_params; also true for a non-generative param under a `.repeat()` element
.has_complete_defaults -> bool
.is_finite -> bool
...
| Method | Semantics |
|---|---|
| `.slice(values=None, **kw)` | Remove params; **substitute** the value at every reference site — conditions, constraint expressions (bound-origin included, envelopes recompute on re-resolution), and `.repeat()` counts — then statically resolve (below) and re-resolve |
| `.freeze(values=None, **kw)` | Fix values, keep params in output; conditions and counts resolve statically (below) |
...
**Static resolution (D-92).** Substitution is only half of either operation. Once every param a piece of *derived* structure reads is determined, that structure is no longer derived, and both operations **fold** it to its canonical static form: a `.repeat()` count becomes a static **`int`**, and a condition that folds to `True` becomes **no condition** (`ParamDef.condition is None`). Folding to the canonical form rather than to a constant expression is the operative part — `has_variable_length`, `coordinate_paths()`'s row-33 check, `cardinality()`, and the `Array`-vs-`List` dtype rule all test the static *form*, so a `Literal(3)` count would leave every one of them misreporting the space. It is also what makes `coordinate_paths()` reachable at all for a space declared with a param-driven count: `.slice()`-ing that count is the route to a fixed layout.

A `True` fold drops the condition, but a **`False` fold is deliberately left in place** — an always-active param *is* an unconditional one, so dropping the condition is information-preserving, whereas removing a permanently-inactive param would take a declared name out of the path namespace that `.params`, `flatten`, and the fingerprint preimage all observe.

The two operations fold with different reach, and the difference is exactly what distinguishes them. **`.slice()` folds unconditionally** — the param is gone, so no config can hold another value for it. **`.freeze()` folds only where the frozen param's own domain admits a single value** (real/integer narrowed to `lo == hi`, categorical/ordinal to one value): it *keeps* the param, and the kinds it pins by a hard `require` instead of by domain narrowing — bool, choice, subset, permutation, custom, program (see the per-kind mechanism below) — retain a domain still admitting their other values, so a config may legally hold one and merely be infeasible. Folding there would report a param active where evaluation says it is not, and would drop a frozen choice's variant-activation condition, breaking that freeze's fingerprint equality with its hand-written pin-and-prune expansion. A count is integer-typed by row 12, so its referent is always domain-narrowed and always folds.

The fold is **best-effort over a closed set**: the reference-free expressions the ordinary evaluator can evaluate against an empty config, excluding any reaching a `ds.value` or `.prop()` leaf (`fn`'s calling convention promises a call with the operand values *at evaluation* — a structural operation is not a call site it agreed to). Anything unfoldable stays an expression, which is always sound: the lift merely stays dynamic. Derived-space fingerprints move accordingly — `freeze(n=3)` is fingerprint-equal to the hand-written `integer(3,3).default(3)` + `.repeat(3)` spelling, which is correct, those two spaces having identical valid-config sets.
...
`.slice()` does not support a custom param (no substitution target for a `.prop()` expression's operand) — a path-named resolution error. A `.symbolic()`/`.code()` param has no `.prop()` and so no such obstruction — `.slice()` supports it like any ordinary leaf.
...
suite as a tool, since a supplied morphism has no other way to be shown sound. It additionally
round-trips the source's **authored** phenotypes — every anchor, and `apply_defaults({})` —
under the law name `round_trip_declared`. Those are the values `encode` exists to carry, and the
only ones that exercise the round-trip's tolerance: a phenotype obtained from `decode` sits on
the chart's image, so `encode` recovers the very unit coordinate it came from and the comparison
is exact, whereas an authored `1e-3` under a `Log()` chart is recovered only to within
floating-point accuracy. A consequence worth stating, because it is silent: `config_hash` is
exact, so a phenotype that has been through `encode`/`decode` may hash differently from the one
you started with — key observations on the phenotype you hold, not on a round-tripped copy.
...
**`Primitive`.** `arity` is an int (exact) or a `(lo, hi)` pair (`hi=None` unbounded) — the only place a `.symbolic()` op's argument count is checked at all; a bare string in `primitives` carries no arity and no meaning (core assigns neither — a primitive vocabulary is otherwise fully open, DECISIONS.md D-90). An int and its `(n, n)` spelling are fingerprint-equal. `fn`, when given, is never called by core — it rides the non-serializable set below like `validators`/`.symbolic()`'s `sampler`, degrading in place (raise/mark/drop) rather than poisoning the whole param.
...
**Resolution timing.** Resolution is unspecified relative to construction. A space built in argument position (a choice variant or struct body) may carry a `.when()` condition — **or a `.repeat()` count** — that references a param binding only in an *enclosing* scope — the sole scoping rule's up-walk — which cannot resolve while that payload is built standalone. Reference, type, and cycle checks (rows 6, 7, 12, 14) over such conditions and counts are therefore deferred to a finalization pass over the fully-merged space, and any resulting error surfaces no later than the **first terminal operation** — `sample`, `sample_one`, `validate`, `validate_param`, `evaluate_constraints`, and (once implemented) `fingerprint`, `to_json`, and every introspection surface must trigger this finalization. The error is still a `ResolutionError` (phase R), computed from space structure alone with no config; only its timing moves. Constraint (`.forbid()`/`.require()`/`.encourage()`/`.discourage()`) references stay strict and raise eagerly, since cross-scope constraints use the down-reference-at-the-common-ancestor route instead. **Expression-bound** references are likewise eager (never deferred): the bound's chart envelope must be computed during the declaring scope's own resolution, before any enclosing scope merges, so a bound expression tolerates no enclosing-scope up-reference — a cross-scope bound coupling is written by hand at the common ancestor (see *Expression bounds are sugar*). A **repeat count** deliberately falls on the *deferred* side of exactly that line (D-91): counts are runtime-evaluated, so no chart or other in-scope consumer forces the check early, and the count's own row-12 calculus is a check rather than a consumer — a check that can move. This is what makes "joins the dependency graph and cycle check … exactly as a condition does" (see *Modifiers and Layering*) true of timing as well as of ordering.

**Relocation rewrites every reference a param carries, not only `ParamDef.condition`.** A lift keeps two of them inside its `ListDomain` — the `count` expression and the per-element constraint templates — so merging a child space under a struct/variant/lift prefix must reprefix those by the same rename. Both fail *silently* otherwise, which is why this is stated rather than left to implementation: a dangling count reads as Kleene-Unknown-from-inactivity and materializes `[]` (the count rule under *Defaults*), and a dangling element constraint goes inapplicable under Kleene rule 4 — a hard `.forbid()` that stops deciding feasibility while `validate()` still reports `valid`. The finalization pass audits both stores for exactly this reason.
...
| 34 | A `struct`/`choice` element nested under more than one `.repeat()` level, by either the chained or the compositional route (see *Modifiers and Layering*) | R |
...
- **Static resolution:** `.slice()`/`.freeze()` fold the derived structure a fixed value determines (D-92) — a count to a static `int` (so `has_variable_length`, `coordinate_paths()`, `cardinality()`, and the `Array` dtype rule all agree, and slicing a count is the route to a fixed layout), an always-true condition to `None`, with a `False` fold deliberately left in place; `.slice()` folds unconditionally while `.freeze()` folds only where the domain admits a single value, so a constraint-pinned kind (bool, choice, …) keeps its condition and a frozen choice stays fingerprint-equal to its pin-and-prune expansion; `freeze(n=3)` is fingerprint-equal to the hand-written static spelling; an unfoldable or `ds.value`-reaching expression is left alone and `fn` is never called.
- **Reference closure:** every param reference a resolved space stores names something declared in that space — over all four stores (`Space.conditions`/`ParamDef.condition`, `Space.constraints`, `ListDomain.count`, `ListDomain.element_constraints`, the last two at every lift level), asserted against the same predicate row 6 uses, and swept over every corpus fixture plus the full product of reference-carrying declaration × nesting route. This is the class-level law behind the relocation bugs: each one stored a pre-relocation path that bound to nothing, and each failed *permissively* (Kleene-Unknown from apparent inactivity) rather than raising. A lifted choice's discriminator template (`pipe[]`) and an instance path (`stops[0].dwell`) both resolve through their owning lift, so neither being a `params` key of its own is not a violation.
- **Structure:** **relocation preserves domain-carried references** — a lift's `count` and its per-element constraint templates keep binding to the params they named through every relocation route (struct body, choice variant payload, lifted-struct element) and through chained lifts at every level, so realized length equals the count param's value and a per-element hard constraint keeps deciding feasibility; `dependency_graph` names only real `space.params` keys, which is where a dangling reference is observable before it reaches a sample; row 6 is enforced over both stores at finalization, so a typo raises instead of silently going Unknown; a count's *enclosing-scope* reference is deferred, not eagerly rejected (D-91), and a cross-scope cycle through one is still row 7. `unflatten(flatten(c)) == c`; per-element constraint instantiation counts; `Array`-vs-`List` dtype per static/dynamic count level; leaf-flattening aggregate values on nested lifts; **the fixed leaf layout** — `coordinate_paths()` round-trips through `unflatten` with no bookkeeping keys present (`ds.unflatten(dict(zip(space.coordinate_paths(), values)), space)`) on a space where every `.repeat()` count is a literal integer and no param carries a condition, raises a path-named resolution error (row 33) otherwise, excludes lift-length bookkeeping at every nesting depth, and orders its output identically to `flatten`'s (and therefore the DataFrame's) leaf order.
- **Representation:** `decode` totality as **domain membership** — `source.validate(rep.decode(g)).param_errors == ()` for every `g` drawn from `target` (not `.valid`, which folds in feasibility, and so would be false by construction wherever a constraint is opaque); `encode` target-validity when `invertible`; **defaults and anchors** carried into the target are valid there, and every one no `encode` could carry appears in `dropped_defaults`/`dropped_anchors` rather than crossing unencoded; the **one-directional** round-trip `decode(encode(x)) == x` — **up to floating-point accuracy** (`rel_tol = abs_tol = 1e-9`, the same tolerance convention grid membership uses), not bit-exactly, since a `Log()`/`Logit()` chart composes `to_unit`/`from_unit` through `log`/`exp`, which are not exact inverses at IEEE-754 precision (D-94) — with `encode(decode(g)) == g` explicitly *not* a law (integer charts, quantized grids, one-hot ties, and random-key permutations are all many-to-one); the round-trip is checked over **authored** phenotypes as well as decoded ones, since only an authored value exercises the tolerance at all; **feasibility agreement** `target.is_feasible(g) == source.is_feasible(rep.decode(g))`, unconditional for every representation `represent()` successfully builds, because transport is total over that set (the one narrow exception — a dynamic-count lift touched by an expression whose encoding supplies neither `decode_expr` nor `rewrite` — is a build-time error, never a silently-unsound representation); the **identity** — a rule set matching no param leaves `target.fingerprint() == source.fingerprint()` at both scopes with `decode(c) == c == encode(c)`; `then` **associative** with identity a two-sided unit — asserted on target fingerprints *and* on decoded values, over **derived** representations, which requires representing a representation target (`rep.target` is an ordinary `Space`, so transport must handle the `ChartApply` node a representation itself emits — without it `then` is unusable past one derived level, and the failure is at *build* time, not compose time); **path and arity preservation** for derived representations — `set(target.params) == set(source.params)` over definition-path keys, dimensionality unconstrained, a param with relocated descendants or a count dependent never encoded, and a `.prop()` dependent encoded only when the matched `Encoding` supplies `prop_expr()`; the **induced chart representation** touches exactly the chart-bearing params (own *or* element level) that no count or `.prop()` reads, targets `real(0,1)` with `periodic` mirrored, and is measure-preserving; `Representation` never enters the IR, `to_json`, or the preimage.
...
- **Program types:** a `.symbolic()` value validates iff every `"op"` is a name this param declared, every `"var"` is a `signature.args` key, every `"const"` lies within some declared literal's bounds, and depth `<= max_depth` (a leaf is depth 1) — vocabulary is open at *declaration* time (any string names a primitive, D-90) but still checked at *value* time; arity binds only where a `Primitive` declares one (a bare string accepts any arity), and an exact-int arity is fingerprint-equal to its `(n, n)` range spelling (D-89); `validators` run only after the structural check passes, and a raising validator never escapes a public call; `.code()` is always non-generative, `.symbolic()` is non-generative unless `sampler=` is given, and `has_nongenerative_params` is true for either kind under a `.repeat()` element as well as at the top level; per-field opacity (D-88) — `validators`/`.symbolic()`'s `sampler`/`Primitive.fn` each ride raise/mark/drop *in place*, degrading only that field, never the whole domain, and `from_json` raises on a marked field; `.freeze()` is fingerprint-equal to the hand-written `require(p == value)` + `.default(value)` expansion, and — unlike a custom type — `.slice()` supports a program param unconditionally, since its value is always a plain, comparable, serializable JSON dict.
