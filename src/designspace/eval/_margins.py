...
from designspace.expr import BoolExpr, BoolOp, Compare, Not
...
def margin(
    ...
    result = _margin(expr, config, activity, space)
...
def _margin(
    ...
    if isinstance(expr, BoolOp):
        left_m = margin(expr.left, config, activity, space)
        right_m = margin(expr.right, config, activity, space)
        if left_m is None or right_m is None:
            return None
        return min(left_m, right_m) if expr.op == "and" else max(left_m, right_m)
    if isinstance(expr, Not):
        inner = margin(expr.operand, config, activity, space)
